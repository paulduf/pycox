...
def administrative_scores(func):
    ...
    def metric(time_grid, durations, durations_c, events, surv, index_surv, reduce=True, steps_surv='post'):
        ...
        assert (durations[events == 0] == durations_c[events == 0]).all(
        ), 'Censored observations need same `durations` and `durations_c`'
        assert (durations[events == 1] <= durations_c[events == 1]).all(
        ), '`durations` cannot be larger than `durations_c`'
        idx_ts_surv = utils.idx_at_times(
            index_surv, time_grid, steps_surv, assert_sorted=True)
        scores, norm = _admin_scores(
            func, time_grid, durations, durations_c, events, surv, idx_ts_surv)
        if reduce is True:
...
def _admin_scores(func, time_grid, durations, durations_c, events, surv, idx_ts_surv):
    ...
    for i in numba.prange(n_times):
        ...
        _single(func, ts, durations, durations_c, events,
                surv, idx_ts_surv_i, scores_i, n_indiv)
...
def _integrated_admin_metric(func):
    def metric(time_grid, durations, durations_c, events, surv, index_surv, steps_surv='post'):
        scores = func(time_grid, durations, durations_c, events,
                      surv, index_surv, True, steps_surv)
...
integrated_binomial_log_likelihood = _integrated_admin_metric(
