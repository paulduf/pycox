...
def _sum_concordant(s, t, d):
    ...
    for i in numba.prange(n):
        for j in range(n):
            if j != i:
                count += _is_concordant(s[i, i], s[i, j],
                                        t[i], t[j], d[i], d[j])
    return count
...
def _sum_concordant_disc(s, t, d, s_idx, is_concordant_func):
    ...
    for i in numba.prange(n):
        ...
        for j in range(n):
            if j != i:
                count += is_concordant_func(s[idx, i],
                                            s[idx, j], t[i], t[j], d[i], d[j])
    return count
...
def concordance_td(durations, events, surv, surv_idx, method='adj_antolini'):
    ...
    assert type(durations) is type(events) is type(
        surv) is type(surv_idx) is np.ndarray
